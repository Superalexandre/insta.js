...
ClientUser.prototype.postPhotoInStory = async function(buffer, media) {
...
ClientUser.prototype.postStory = async function(buffer) {
    const story = await this.client.ig.publish.story({
        file: buffer
    })
...
ClientUser.prototype.postPhoto = async function(buffer, caption, location, usertags) {
...
ClientUser.prototype.findLocation = async function(lat, lng, query) {
    const locations = await this.client.ig.search.location(lat, lng, query ?? "")
    return locations
}

ClientUser.prototype.tagUser = async function(users) {
