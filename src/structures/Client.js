import { withRealtime, withFbns } from 'instagram_mqtt'
...
class Client extends EventEmitter {
    ...
    async editProfile (options) {
        const newProfile = await this.ig.account.editProfile(options)
    ...
    async postStory ({ buffer, stickers }) {
        const stickerBuild = new StickerBuilder()
    ...
    async postPhoto ({ buffer, caption, location, usertags }) {
        const media = await this.ig.publish.photo({
            file: buffer,
            caption: caption ?? "",
    ...
    async findLocation ({ lat, lng, query }) {
        const locations = await this.ig.search.location(lat, lng, query ?? "")
        return locations
    }
    ...
    async tagUser (users) {
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            const generatedUserTag = await generateUsertagFromName(this.ig, user.username, user.posX, user.posY)
    ...
    async postAlbum ({ buffers, caption, location }) {
        const media = await this.ig.publish.album({
            items: buffers,
            caption: caption ?? "",
            location: location ?? undefined,
        })
    ...
    async login (username, password, state, proxy) {
        ...
        ig.fbns.addListener('data', (data) => this.handleFbnsReceive(data))
